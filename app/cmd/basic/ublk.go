	...
	return cli.Command{
		...
		Action: func(c *cli.Context) {
			if err := ublkCreateTarget(c); err != nil {
				logrus.WithError(err).Fatalf("Failed to run UblkCreateTargetCmd")
			}
		},
	}
}
...
func ublkCreateTarget(c *cli.Context) error {
	...
	err = spdkCli.UblkCreateTarget(c.String("cpumask"), c.Bool("disable-user-copy"))
	if err != nil {
		return err
	}
	return nil
	...
	return cli.Command{
		...
		Action: func(c *cli.Context) {
			if err := ublkDestroyTarget(c); err != nil {
				logrus.WithError(err).Fatalf("Failed to run UblkDestroyTargetCmd")
			}
		},
	}
}
	...
	return cli.Command{
		...
		Action: func(c *cli.Context) {
			if err := ublkGetDisks(c); err != nil {
				logrus.WithError(err).Fatalf("Failed to run UblkGetDisksCmd")
			}
		},
	}
}
	...
	return cli.Command{
		...
		Action: func(c *cli.Context) {
			if err := ublkStartDisk(c); err != nil {
				logrus.WithError(err).Fatalf("Failed to run UblkStartDiskCmd")
			}
		},
	}
}
...
func ublkStartDisk(c *cli.Context) error {
	...
	err = spdkCli.UblkStartDisk(c.String("bdev-name"), c.Int("ublk-id"), c.Int("queue-depth"), c.Int("num-queues"))
	if err != nil {
		return err
	}
	return nil
	...
	return cli.Command{
		...
		Action: func(c *cli.Context) {
			if err := ublkRecoverDisk(c); err != nil {
				logrus.WithError(err).Fatalf("Failed to run UblkRecoverDiskCmd")
			}
		},
	}
}
...
func ublkRecoverDisk(c *cli.Context) error {
	...
	err = spdkCli.UblkRecoverDisk(c.String("bdev-name"), c.Int("ublk-id"))
	if err != nil {
		return err
	}
	return nil
	...
	return cli.Command{
		...
		Action: func(c *cli.Context) {
			if err := ublkStopDisk(c); err != nil {
				logrus.WithError(err).Fatalf("Failed to run UblkStopDiskCmd")
			}
		},
	}
}
...
func ublkStopDisk(c *cli.Context) error {
	...
	err = spdkCli.UblkStopDisk(c.Int("ublk-id"))
	if err != nil {
		return err
	}
	return nil
